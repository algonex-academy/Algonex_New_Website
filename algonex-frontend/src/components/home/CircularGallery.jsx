...
class Title {
  ...
  createMesh() {
    ...
    // Set local scale and position assuming parent scale y and scale x are 1.
    // To prevent horizontal stretching when the parent is scaled to imgAspect,
    // we pre-divide the local text width by the default fallback aspect ratio (16/9).
    const textHeight = 0.15;
    const textWidth = textHeight * this.aspect / (16 / 9);
...
class Media {
  constructor({
    ...
    this.font = font;
    this.aspectRatio = 16 / 9; // Fallback before image loads
    this.createShader();
    this.createMesh();
    this.createTitle();
    this.onResize();
...
class App {
  ...
  layoutMedias() {
    ...
    this.medias.forEach((media) => {
      ...
      const imgAspect = 16 / 9; // Fixed landscape 16:9 aspect ratio
      
      // Calculate plane scale y (using base height factor 810 to fill container vertically)
      media.plane.scale.y = (media.viewport.height * (810 * scale)) / this.screen.height;
      media.plane.scale.x = media.plane.scale.y * imgAspect;
    ...
    const widthTotal = this.medias.reduce((sum, media) => sum + media.width, 0);
    ...
    this.medias.forEach((media, index) => {
      ...
      if (index === 0) {
        ...
        const prevMedia = this.medias[index - 1];
        media.x = prevMedia.x + (prevMedia.width + media.width) / 2;
      }
    });
  }
  onCheck() {
    ...
    this.medias.forEach(media => {
      const targetPos = media.x - media.extra;
      const distance = Math.abs(this.scroll.target - targetPos);
  ...
  onResize() {
    ...
    if (this.medias) {
      this.medias.forEach(media => {
        media.screen = this.screen;
        media.viewport = this.viewport;
        if (media.plane.program.uniforms.uViewportSizes) {
          media.plane.program.uniforms.uViewportSizes.value = [media.viewport.width, media.viewport.height];
        }
      });
      this.layoutMedias();
    }
