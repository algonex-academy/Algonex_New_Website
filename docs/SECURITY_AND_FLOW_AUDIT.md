...
- Backup/restore APIs authorize by `is_staff OR role=='admin'` — left as-is (not a bug; changing it risks locking out an admin). (MEDIUM)
- EmailLog dashboard mislabels recipient totals / over-counts BCC as delivered. (LOW)
- **Orphaned media** — replacing a photo / re-registering leaves the old file on disk (disk hygiene). (MEDIUM)
...
- ✅ **ProfilePage** payment-submit error now extracts a string from the DRF error object (commit `71dfb8d`).
- ✅ **Catalog pages** request `page_size=50` so items beyond the first 10 are reachable (commit `c3170c0`).
- ✅ **`/register/step2/` IDOR closed** — step 1 issues a short-lived signed token bound to the email; step 2 requires it (commit `2cecefc`). Was previously flagged as needing design; implemented with `django.core.signing`, no flow disruption. New negative test asserts step 2 without a token is rejected.

### Still needs a product / infra decision
- **Private media storage** (HIGH) — moving ID cards/invoices/resumes/photos off public `/media/` to private S3 + signed URLs changes how every uploaded file is served; needs a storage decision. (Execution-hardening headers already shipped.)
